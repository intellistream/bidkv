---
name: bidkv
description: "BidKV 仓库开发助手 — framework-portable KV cache compression scheduling 原语。用于实现 BidKV solver、baseline 策略、vLLM/SGLang 适配器、实验框架。零外部依赖，SC 2026 论文实施。"
...
| 执行语义     | Mode A (recompute) / Mode B (truncation) | native token-level / request-level fallback |
| 产出物       | Table 1 + Fig 3/4/5/6                 | Table 2 + Fig 7                     |
| claim 角色   | Scenario A 核心主张                    | directional consistency + portability |

### vLLM 双轨模式

- **Mode A (P0)**：BidKV selection + vLLM native preempt/recompute。不修改 vLLM 源码。
- **Mode B (P1)**：最小 vLLM 内核扩展 ~60-80 LOC（`truncate_request_tail()` API）。
- Mode B 在 Gate-B deadline 前未稳定 → 论文使用 Mode A 数据。
...
- `config.py`：新增 `execution_mode: str = "recompute_fallback"` 字段（可选值：recompute_fallback / tail_truncation）
- `adapters/vllm/adapter.py`：`execute_compression()` 按 `execution_mode` 路由；新增 `_execute_recompute_fallback()` — 调用 `scheduler.abort_requests()`
