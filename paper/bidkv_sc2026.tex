...
\begin{subfigure}[b]{0.47\textwidth}
  \includegraphics[width=\linewidth]{figures/fig1_motivation_1panel.pdf}
  \caption{Active requests exhibit extreme heterogeneity in reclamation
    opportunity: $\max R / \min R$ across live victim candidates at each
    KV-pressure event (250 events, mixed ShareGPT, rate\,=\,3.8\,req/s).
    Spread exceeds $10{\times}$ in 99.6\% of events; median is $128{\times}$.}
  \label{fig:intro:hetero}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.50\textwidth}
  \includegraphics[width=\linewidth]{figures/fig1_intro_evidence_panel_b.pdf}
  \caption{Victim-selection trade-off space across 150 KV-pressure snapshots
    (mixed ShareGPT, rate\,=\,3.8\,req/s).
    Each point is one strategy's chosen victim at one snapshot: X\,=\,completion
    ratio $c$ (recompute cost proxy; 0\,=\,newest), Y\,=\,KV-footprint rank
    (1\,=\,largest available).
    Filled ellipses show 1.5$\sigma$ concentration regions; diamonds mark means.
    The \textbf{ideal zone} (green shading) is upper-left: high KV freed at low
    recompute cost.
    LIFO\,(red) sits in the lower-left — low KV recovery ($\bar{r}\!=\!0.21$), wasting
    each preemption opportunity.
    Largest-first\,(blue) reaches the top edge ($\bar{r}\!=\!1.00$) but drifts
    rightward ($\bar{c}\!=\!0.74$), incurring unnecessary recompute cost.
    BidKV\,(yellow) lands firmly in the ideal zone ($\bar{r}\!=\!0.96$,
    $\bar{c}\!=\!0.64$): near-maximum KV recovery at lower cost than
    Largest-first — not a compromise, but a dominant trade-off point.}
  \label{fig:intro:lifo}
\end{subfigure}
\caption{\textbf{The victim-selection problem under KV pressure.}
  (a)~Reclamation opportunity spans orders of magnitude at every pressure event.
  (b)~Victim-selection trade-off space: X\,=\,recompute cost ($c$),
  Y\,=\,KV footprint rank.
  LIFO wastes KV capacity ($\bar{r}\!=\!0.21$); Largest-first maximises KV but
  ignores cost ($\bar{c}\!=\!0.74$); BidKV occupies the ideal upper-left zone
  ($\bar{r}\!=\!0.96$, $\bar{c}\!=\!0.64$), dominating both baselines.}
\label{fig:intro_evidence}
\end{figure*}

Because reclamation costs are heterogeneous---as
Figure~\ref{fig:intro_evidence} concretely demonstrates---the scheduler needs
...
\paragraph{Formalizing the Heterogeneity.}
As motivated by the extreme spread observed in
Figure~\ref{fig:intro_evidence}(a), we define a method-agnostic
\emph{reclamation-opportunity score}
\begin{equation}
R_i = B_i \cdot (1 - p_i)
\label{eq:reclamation_opportunity}
\end{equation}
where $B_i$ is the number of KV tokens currently held by candidate~$i$
(freed in full upon preemption under recompute-fallback semantics) and
$p_i = g_i / G_i$ is the post-hoc completion fraction ($g_i$ = output tokens
generated so far, $G_i$ = realized final output length).  This score is
\emph{analysis-only} and \emph{method-agnostic}: it references no scheduling
formula and captures the intuition that a favorable eviction target should both
free substantial KV space and have invested relatively little decode work.

The heterogeneity is pervasive in practice.  Across 250 KV-pressure events
from a mixed-length ShareGPT run (rate~=~3.8\,req/s), in 99.6\% of events the
highest- and lowest-opportunity candidates differ by at least $10{\times}$ in
$R$ ($\max R / \min R \geq 10$); the median spread is $128{\times}$, and
94.8\% of events exceed $20{\times}$.  This spread arises from two orthogonal
dimensions: KV footprint varies widely across requests of different prompt
lengths, and completion fraction is spread across $[0,1]$ as requests enter
and exit at different lifecycle stages.  Order-only heuristics that ignore both
dimensions---such as LIFO, which selects the most recently admitted
request---therefore routinely encounter events where the chosen victim is far
from the highest-opportunity candidate under this proxy
(Figure~\ref{fig:intro_evidence}(b)).  The victim-selection problem is
therefore not merely theoretical; it appears repeatedly in realistic traces,
and this heterogeneity motivates the two capabilities identified above:
explicit per-request reclamation-cost signals and cross-request coordination.
...
resources.
The knapsack literature~\cite{Martello1993, Kellerer2004} provides algorithmic
...
$\tokfree$ equal to its current KV footprint (prompt plus tokens generated
so far, sampled at scheduling time).  $\qualdelta$ is computed as follows.
The disruption estimate anchors at $1$ and adds two
...
(empirically validated in Section~\ref{sec:eval:robustness}).
...
dependencies} (Python $\geq$~3.10 standard library only).  The system defaults to \textbf{disabled}
...
\item \textbf{Pre-schedule.}  Before each native scheduling call, the hook
  ...
  PE, PE-SJF, and \bidkv skip this step, relying instead on native reclamation triggers.
...
(Section~\ref{sec:eval:mechanism}), (5)~framework portability on SGLang
(Section~\ref{sec:eval:sglang}), and (6)~robustness to hyperparameter variations
...
\item \textbf{\bidkv}: SJF admission, pressure-gated running reorder
  ...
  victim ordering; proactive reclamation explicitly disabled
  (relies entirely on utility-ranked native preemption);
...
(68.4\%) but below strategies with managed cross-request victim selection, confirming that
admission ordering alone is insufficient for maximum SLO.

\emph{High load (rate\,=\,5.7):}
All strategies degrade under intense pressure.  Static-Random leads on SLO
(82.0\%); \bidkv achieves the second-best SLO (79.9\%) and second-best
TTFT~P95 (744\,ms).  PE-SJF achieves the lowest TTFT~P95 (692\,ms) through
conservative scheduling (no proactive preemption), but at lower throughput
(3.53 vs.\ \bidkv 3.60\,req/s) and lower SLO (78.1\%).
Static-Random achieves the highest throughput (4.14\,req/s, $+$15\% over
\bidkv) but its TTFT~P95 is 2.0$\times$ higher than \bidkv's
(1{,}499\,ms vs.~744\,ms).

\paragraph{Cross-rate summary.}
Averaging across all three rates, \bidkv achieves the best TTFT~P95
(544\,ms, 45\% below Static-Random's 994\,ms) and second-best SLO (86.7\%,
0.8\,pp below Static-Random's 87.5\%) at a throughput of
2.99\,req/s---7.1\% below Static-Random's 3.22\,req/s.  This tradeoff
...
Static-Random, 20\,\% better than PE-SJF), with competitive SLO
...
\item \textbf{PE-SJF} (336 events, 366k tokens, 1{,}087\,tokens/event) also
  ...
  that the LIFO victim tends to carry more KV blocks, giving higher
  freed-per-event than PE (1{,}087 vs.~794).  Its total recompute volume
  (366k tokens, highest across all strategies) drives the worst
  TPOT~P95 (153.5\,ms) and SLO 3.6\,pp below \bidkv (79.6\,\%
  vs.~83.2\,\%), as sustained recomputation intermittently delays decode
  completion.
...
\item \textbf{Static-Random} (329 events, 88k tokens, 269\,tokens/event) has
  ...
  (more than Largest-First's 296 and \bidkv's 181),
  it frees only 88k tokens in total---35\,\% of \bidkv's 253k.  The KV capacity actually recovered
...
\item \textbf{Largest-First} (296 events, 325k tokens, 1{,}100\,tokens/event)
  ...
  often near-complete long requests.  Preempting and recomputing these targets
  consumes prefill bandwidth and delays queued arrivals, suppressing SLO
  (79.9\,\%, 3.3\,pp below \bidkv) and elevating TTFT~P95
  (677\,ms vs.~\bidkv's 631\,ms, 7\,\% higher), despite comparable
  TPOT~P95 (112.9\,ms vs.~\bidkv's 107.2\,ms).
...
\item \textbf{\bidkv} (181 events, 253k tokens, 1{,}398\,tokens/event) fires
  ...
  rate\,=\,3.8, 5\,\% below PE-SJF's 666\,ms) and second-best SLO
  (83.2\,\%, 0.9\,pp below Static-Random) despite issuing zero proactive
  preemptions.
\end{itemize}
...
\subsection{Robustness to Hyperparameter Variations}
\label{sec:eval:robustness}

We evaluate the sensitivity of \bidkv to its core configuration parameters: the completion weight ($c_w$), the starvation penalty ($s_w$), and the KV-utilization threshold for gating the running-queue reorder ($Gate$). Table~\ref{tab:sensitivity} presents the serving metrics under the medium-load mixed workload (rate\,=\,3.8\,req/s) across a wide sweep of parameter values.

\textbf{Observation.} Varying these parameters produces only minor fluctuations in overall serving performance. SLO attainment remains highly stable between 82.3\,\% and 84.2\,\%, and TTFT P95 fluctuates modestly between 600\,ms and 659\,ms. Across all tested extreme values (e.g., scaling $c_w$ from 0.25 to 2.0), the performance remains substantially superior to the native LIFO baseline (which achieves 68.4\,\% SLO and 6{,}769\,ms TTFT P95).

\textbf{Analysis.} This robust behavior is a direct, empirical validation of \bidkv's ordinal ranking design (Section~\ref{sec:design:scoring}). Because the utility formula $U = r / (\qualdelta + \epsilon)$ relies on relative prioritization rather than absolute cost thresholds, scaling the disruption penalty weights alters the magnitude of $\qualdelta$ but rarely changes the identity of the top-ranked victim at a pressure event. A newly started request with a large KV footprint will consistently rank as a better eviction candidate than a near-complete request, regardless of whether $c_w$ is set to 0.25 or 2.0.

From a deployment perspective, this parameter insensitivity is highly desirable. It demonstrates that \bidkv acts as a plug-and-play scheduling layer that delivers substantial admission responsiveness gains out-of-the-box, eliminating the need for system operators to perform workload-specific hyperparameter tuning.

\begin{table}[h]
\centering
\small
\caption{Robustness to hyperparameter variations (mixed workload, rate\,=\,3.8). The system exhibits high stability across a wide range of parameter values, confirming the robustness of ordinal utility ranking.}
\label{tab:sensitivity}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l c c c c c c c}
\toprule
\textbf{Variant} & $c_w$ & $s_w$ & \textbf{Gate} & \textbf{Thru} & \textbf{SLO\,\%} & \textbf{TTFT P95} & \textbf{TPOT P95} \\
\midrule
default $\star$ & 0.5 & 0.3 & 0.95 & 3.39 & 82.3 & 633\,ms & 109.6\,ms \\
\midrule
cw\_0.25 & 0.25 & 0.3 & 0.95 & 3.34 & 82.8 & 631\,ms & 112.0\,ms \\
cw\_1.0 & 1.0 & 0.3 & 0.95 & 3.43 & 84.2 & 600\,ms & 100.7\,ms \\
cw\_2.0 & 2.0 & 0.3 & 0.95 & 3.35 & 83.1 & 659\,ms & 108.2\,ms \\
\midrule
sw\_0.1 & 0.5 & 0.1 & 0.95 & 3.40 & 83.5 & 636\,ms & 104.3\,ms \\
sw\_0.6 & 0.5 & 0.6 & 0.95 & 3.36 & 83.2 & 654\,ms & 105.4\,ms \\
sw\_1.0 & 0.5 & 1.0 & 0.95 & 3.42 & 83.9 & 640\,ms & 102.4\,ms \\
\midrule
gate\_0.85 & 0.5 & 0.3 & 0.85 & 3.46 & 83.7 & 640\,ms & 103.3\,ms \\
gate\_0.90 & 0.5 & 0.3 & 0.90 & 3.42 & 83.1 & 637\,ms & 102.4\,ms \\
gate\_0.98 & 0.5 & 0.3 & 0.98 & 3.40 & 83.2 & 621\,ms & 104.4\,ms \\
\bottomrule
\end{tabular}%
}
\vspace{2pt}
{\footnotesize $\star$ Default configuration ($c_w{=}0.5$, $s_w{=}0.3$, $Gate{=}0.95$) matching the main experiments (Table~\ref{tab:rate_full}); independently executed sensitivity runs---minor run-to-run variance expected.}
\end{table}
...
reclamation-cost oracle.  Exact cost prediction is unnecessary (as confirmed by the robustness evaluation in Section~\ref{sec:eval:robustness}): under
...
mixed-length ShareGPT workloads, we showed that \bidkv achieves the best cross-rate TTFT~P95 (544\,ms) and
second-best SLO attainment (86.7\%, 0.8\,pp below Static-Random),
outperforming both capacity-greedy heuristics and default scheduling
baselines at a modest throughput cost (${\sim}$7\%).  At low load, \bidkv leads on all four metrics.
