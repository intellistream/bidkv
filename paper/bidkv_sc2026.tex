...
% ============================================================
\begin{abstract}
Online large language model (LLM) serving faces dynamic KV-cache pressure from heterogeneous request lengths and bursty arrivals. When memory demand exceeds capacity, serving engines must reclaim KV cache from active requests, but existing policies rely on coarse request-order heuristics or local token-scoring rules with limited cross-request awareness. We present BidKV, a utility-guided reclamation policy for request-level victim selection. BidKV estimates, for each candidate action, the KV capacity recovered and the disruption cost, then prioritizes requests by recovered capacity per unit cost. The current prototype instantiates disruption cost using request-lifecycle proxies (completion progress, prompt length, and preemption history) rather than application-provided bids. BidKV is portable across frameworks via a four-layer adapter and integrates non-invasively with vLLM and SGLang. With Llama-3.1-8B-Instruct on an NVIDIA RTX A6000 using ShareGPT traces under Poisson arrivals, BidKV improves 300\,ms TTFT-SLO attainment by 20.6\% on average over vLLM's native policy, reduces P95 TTFT by 89.6\%, and increases 500\,ms goodput by 27.4\%.
\end{abstract}
...
resource~\cite{Vaswani2017, Pope2023}: new arrivals
...
triggers further reclamation, amplifying tail latency and destabilizing service
behavior.  Active-KV reclamation is therefore a cross-request victim-selection
problem that must account for heterogeneous reclamation costs.
...
differentiation lies entirely in scheduling efficiency.  The value of
coordinated reclamation is operating-regime dependent: under sustained
mixed-length KV pressure, \bidkv's utility-guided selection provides
substantial benefit; under low pressure, victim selection remains governed by the
framework's native ordering, with \bidkv's utility-guided reorder
activating only under sustained high-pressure conditions.
...
request~\cite{Dubey2024}.  vLLM~\cite{Kwon2023} manages this state through
...
While token-level scores can drive fine-grained bid generation (\eg partial
truncation), the recompute-fallback execution model used in this work reclaims
entire requests.  Each running request therefore produces a single bid with
...
\item \textbf{Pre-schedule: admission reorder + victim selection + proactive
  ...
  baseline).
  (d)~\emph{SRPT preemption} (Static-Random, H2O-Style, Uniform only):
  ...
  BidKV and Slack-Aware disable SRPT because recompute cost outweighs
  the scheduling gain.
...
H2O-Style       & SJF  & prio & H2O score & \ding{51} & \ding{51} \\
Uniform         & SJF  & prio & equal     & \ding{51} & \ding{51} \\
Slack-Aware     & EDF  & prio & slack     & \ding{51} & \ding{55} \\
...
\paragraph{Baselines.}
Seven strategies form a layered comparison that separates contributions of
...
\noindent
The progression \emph{PE $\to$ PE-SJF $\to$ H2O-Style $\to$ \bidkv}
incrementally introduces SJF admission ordering, proactive reclamation with
quality-aware victim selection, and coordinated bid-based selection.  Note
...
\item \textbf{\bidkv achieves the best TTFT P95 and second-best SLO
  at rate\,=\,3.8.}
  TTFT~P95 is 631\,ms, 5\% below PE-SJF (666\,ms) and 7\% below H2O
  (677\,ms).  SLO(300ms) reaches 83.2\%, 0.9\,pp below Static-Random
  (84.1\%) but 3.3\,pp above H2O (79.9\%) and 14.8\,pp above PE (68.4\%).

\item \textbf{SRPT-enabled strategies lead on throughput metrics.}
  Static-Random and Uniform, which enable SRPT aggressive preemption
  (KV\,$>$\,80\%), achieve the highest Goodput (3.25 and 3.18\,req/s
  vs.\ \bidkv 3.16), lowest TPOT P95 (91 and 92\,ms vs.\ \bidkv
  107\,ms), and lowest Normalized Latency (60.4 and 60.8 vs.\ \bidkv
  65.2).  However, their TTFT P95 is ${\sim}$2$\times$ higher (1216
  and 1189\,ms vs.\ \bidkv 631\,ms), indicating aggressive throughput
...
\item \textbf{\bidkv provides the best latency--throughput balance.}
  ...
  PE-SJF, H2O), \bidkv achieves the highest Goodput (3.16 vs.\ 2.97
  and 2.67) and SLO (83.2\% vs.\ 79.9\% and 79.6\%).  The SRPT
  strategies trade ${\geq}$1.2\,s TTFT P95 for higher throughput---a
...
(2)~\emph{PE-SJF $\to$ H2O-Style} ($+$0.3\,pp SLO at 300ms):
  adding proactive reclamation and attention-based victim selection;
...
(3)~\emph{H2O-Style $\to$ BidKV} ($+$3.3\,pp SLO at 300ms):
...
\caption{Rate sensitivity (mixed workload, vLLM).  (a)~Goodput(500ms) vs.\
  request rate: SRPT-enabled strategies (Static-Random, Uniform) lead at
...
achieve $\geq$92\% SLO(300ms), with \bidkv leading at 97.0\% (vs.\ Uniform
96.6\%, Static-Random 96.5\%, PE-SJF 94.2\%).  \bidkv wins all five
metrics at this rate, though the absolute margins are small---the
Goodput spread among the top four strategies is $\leq$0.02\,req/s and
all achieve $\geq$94\% SLO.

\emph{Medium load (rate\,=\,3.8):}
Sustained KV pressure forces proactive reclamation.  This is the primary
differentiating regime.  Static-Random leads on Goodput (3.25), SLO (84.1\%),
TPOT P95 (91\,ms), and Normalized Latency (60.4), all enabled by SRPT
aggressive preemption.  However, its TTFT P95 (1216\,ms) is nearly 2$\times$
that of \bidkv (631\,ms), which achieves the best tail latency while
maintaining competitive SLO (83.2\%, second overall, $+$3.3\,pp over H2O).
Among non-SRPT strategies, \bidkv leads on all five metrics, confirming that
coordinated utility-based victim selection adds value beyond per-request
heuristics.

\emph{High load (rate\,=\,5.7):}
Pressure is intense; all strategies degrade.  SRPT-enabled strategies
(Uniform, Static-Random) lead on throughput: Uniform achieves the highest
Goodput (3.57) and SLO (81.6\%); Static-Random follows (3.56, 80.4\%).
\bidkv (3.26, 81.0\%) ranks third on Goodput and second on SLO.
For TTFT P95, PE-SJF's zero-eviction approach yields the best tail latency
(727\,ms), followed by H2O (793\,ms) and \bidkv (797\,ms)---a
70\,ms gap (9\%) to PE-SJF.  However, PE-SJF's Goodput (2.80) is 14\% below
\bidkv and its SLO (74.6\%) is 6.4\,pp lower, indicating that the tail
...
\item \textbf{PE and PE-SJF} trigger zero proactive evictions, relying
  ...
  in high TTFT and low Goodput.

\item \textbf{H2O-Style} selects victims by per-request attention-weight
  concentration, without considering cross-request recomputation cost.  This
  occasionally targets near-complete requests whose recompute is expensive,
  causing cascading pressure.
...
\item \textbf{\bidkv} uses the utility ratio $U = r / (\delta + \varepsilon)$
  ...
  the best combined Goodput and TTFT~P95.
\end{itemize}
...
Through a seven-strategy evaluation on vLLM with Llama-3.1-8B-Instruct under
...
both attention-based heuristics and default scheduling baselines.  At low
load, \bidkv leads on all five metrics.  At higher load, strategies that
