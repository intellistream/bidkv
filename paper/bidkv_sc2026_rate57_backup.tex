...
\caption{Reclamation event analysis (mixed workload, rate\,=\,5.7).
  Left bars: total reclamation count (all paths: native LIFO + proactive).
  Right bars: total tokens freed ($\times$1000).
  \bidkv issues the fewest events (225) yet achieves the highest
  freed-per-event (1{,}315\,tokens/event), reflecting utility-ranked
...
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{PE} (422 events, 399k tokens, 944\,tokens/event) relies entirely
...
\item \textbf{PE-SJF} (383 events, 366k tokens, 956\,tokens/event) also
  uses only native LIFO reclamation.  SJF admission alters queue composition so
  that the LIFO victim tends to carry more KV blocks, giving slightly higher
  freed-per-event than PE (956 vs.~944).  PE-SJF achieves the lowest
  TTFT~P95 (692\,ms) at this rate---7\,\% below \bidkv's (744\,ms)---through
  SJF admission priority without any proactive component.  However, its high
  total recompute volume (366k tokens, second-highest across all strategies)
  drives the worst TPOT~P95 (129.7\,ms) and SLO 1.8\,pp below \bidkv
  (78.1\,\% vs.~79.9\,\%), as sustained recomputation intermittently
  delays decode completion.

\item \textbf{Static-Random} (454 events, 117k tokens, 259\,tokens/event) has
  the lowest freed-per-event of all five strategies---5.1$\times$ lower than
  \bidkv's 1{,}315\,tokens/event.  Random victim selection frequently targets
  short requests with small KV footprints; despite issuing 454 events
  (more than either other proactive strategy: Largest-First 337, \bidkv 225),
  it frees only 117k tokens in total---40\,\% of \bidkv's 295k.  The KV capacity actually recovered
  per unit of scheduling effort is far lower, a direct consequence of
  ignoring victim heterogeneity.

\item \textbf{Largest-First} (337 events, 341k tokens, 1{,}011\,tokens/event)
  achieves high freed-per-event by targeting the request with the largest KV
  occupancy.  Approximately 126 of these events are proactive preemptions
  ...
  (78.3\,\%, 1.6\,pp below \bidkv) and elevating TTFT~P95
  (879\,ms vs.~\bidkv's 744\,ms, 18\,\% higher), despite comparable
  TPOT~P95 (109.4\,ms vs.~110.4\,ms).

\item \textbf{\bidkv} (225 events, 295k tokens, 1{,}315\,tokens/event) fires
  ...
  progress.  The result is 225 precisely targeted interventions that
  collectively free 295k tokens---fewer total events than any other strategy,
  meaning fewer recompute cycles and less contention for prefill bandwidth.
  This explains why \bidkv achieves second-best TTFT~P95 (744\,ms at
  rate\,=\,5.7, second only to PE-SJF's 692\,ms) and second-best SLO
  (79.9\,\%, 2.1\,pp below Static-Random) despite issuing zero proactive
  preemptions.
\end{itemize}
...
engine~\cite{Zheng2024sglang} at the most stressed rate (5.7\,req/s, mixed
workload).  The SGLang adapter applies \bidkv's complete scheduling
policy---SJF admission ordering, pressure-gated running-queue reorder, and
utility-ranked victim selection---mirroring the vLLM configuration without
any modification to SGLang's internals.

\paragraph{Managed scheduling vs.~native eviction.}
Vanilla SGLang's native LRU eviction yields 55.2\% SLO and
TTFT~P95 of 6{,}246\,ms---reasonable throughput (3.04\,req/s) but severe
admission latency.  Deploying \bidkv's full policy on SGLang raises SLO to
86.9\% ($+$31.7\,pp) and cuts TTFT~P95 to 598\,ms (10.4$\times$ reduction),
...
\bidkv also outperforms Random-Evict (SLO 86.9\% vs.~85.7\%, TTFT~P95
598 vs.~878\,ms), with notably lower run-to-run variance: Random-Evict's
TTFT~P95 spans 460--1{,}700\,ms across three runs, while \bidkv's
range is 475--667\,ms.  Throughput is within 0.7\% (5.53 vs.~5.57\,req/s).
The key portability result is the pattern: on both vLLM and SGLang,
\bidkv's scheduling policy achieves the best SLO and TTFT~P95 at
negligible throughput cost, confirming that the complete policy, including
its integration hooks, transfers across serving frameworks without
...
\vspace{2pt}
{\footnotesize $\star$ Default configuration ($c_w{=}0.5$, $s_w{=}0.3$, $Gate{=}0.95$) matching the main experiments (Table~\ref{tab:strategy_diff}); independently executed sensitivity runs---minor run-to-run variance expected.}
\end{table}
