#!/bin/bash
# SGLang 消融实验 v6: vanilla_sglang vs random_evict vs bidkv @ rate=3.8
#
# 消融链：
#   vanilla_sglang  — pure SGLang native（无任何干预）
#   random_evict    — random victim selection
#   bidkv           — quality-aware victim selection: U = output_tokens/(δ+ε)
#
# v6 变更（相对于 v5@rate=5.7）:
#   - rate=5.7 → rate=3.8（中等压力，更具代表性的工作点）
#
# 运行方法:
#   tmux new-session -d -s sglang_v6 && tmux send-keys -t sglang_v6 \
#   "bash scripts/run_sglang_v6_rate38.sh 2>&1 | tee results/sglang_v6_rate38/run.log" Enter
...
PYTHON="python3"
OUTPUT_DIR="results/sglang_v6_rate38"
mkdir -p "$OUTPUT_DIR"

echo "=== SGLang v6 Native Ablation @ rate=3.8: vanilla → random → bidkv ==="
...
out = os.environ.get("OUTPUT_DIR", "results/sglang_v6_rate38")
