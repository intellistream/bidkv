#!/bin/bash
# SGLang 消融实验 v5: vanilla_sglang vs random_evict vs bidkv
#
# 消融链：
#   vanilla_sglang  — pure SGLang native（无任何干预，Level 2 由 SGLang 自行选择）
#   random_evict    — random victim selection（只改变 victim 选择方式，随机化）
#   bidkv           — quality-aware victim selection（utility U = output_tokens/(δ+ε)）
#
# 对比目标：
#   vanilla → random: 打破默认顺序本身是否有效？
#   random  → bidkv:  质量感知 vs 随机，体现 BidKV scoring 的价值
#
# 运行方法:
#   tmux new-session -d -s sglang_v5 && tmux send-keys -t sglang_v5 \
#   "bash scripts/run_sglang_v5_native.sh 2>&1 | tee results/sglang_v5_native/run.log" Enter
...
PYTHON="python3"
OUTPUT_DIR="results/sglang_v5_native"
mkdir -p "$OUTPUT_DIR"

echo "=== SGLang v5 Native Ablation: vanilla → random → bidkv ==="
echo "Strategies: vanilla_sglang, random_evict, bidkv"
echo "Workload: mixed, rate=5.7, 3 runs per strategy"
echo "Start: $(date)"
...
out = os.environ.get("OUTPUT_DIR", "results/sglang_v5_native")
